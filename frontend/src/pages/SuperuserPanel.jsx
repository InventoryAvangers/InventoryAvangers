...
import {
  ...
  FiShield, FiBarChart2, FiFileText, FiMessageSquare,
...
import { apiGet, apiPost, apiPut, apiDelete, apiPatch, apiErrMsg } from '../api/axios.js';
...
function ShopsTab({ showAlert, loadData, requests, owners, shops }) {
  ...
  const FEATURES = ['inventory', 'pos', 'returns', 'reports', 'employees'];
...
// Tab: Logs
// ─────────────────────────────────────────────────────────────────────────────
function LogsTab({ shops }) {
  const [logs, setLogs] = useState([]);
  const [loading, setLoading] = useState(true);
  const [filters, setFilters] = useState({ storeId: '', action: '', actorRole: '', from: '', to: '' });
  ...
  return (
    <div className="superuser-card">
      ...
      <div className="su-filter-row" style={{ padding: '12px 20px', borderBottom: '1px solid #f1f5f9' }}>
        <select className="su-filter-select" value={filters.storeId} onChange={(e) => setFilters({ ...filters, storeId: e.target.value })}>
          <option value="">All Stores</option>
          {(shops || []).map((s) => <option key={s._id} value={s._id}>{s.name || s.shopName}</option>)}
        </select>
  ...
  return (
    <DashboardLayout>
      ...
      <div className="superuser-page">
        ...
        {loading && tab !== 'logs' && tab !== 'comms' && tab !== 'inbox' ? (
