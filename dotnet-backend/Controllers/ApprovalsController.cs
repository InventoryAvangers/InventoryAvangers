...
public class ApprovalsController : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateApproval(string id, [FromBody] UpdateApprovalRequest req)
    {
        ...
        if (approval.Action == "delete_product" && req.Status == "approved")
        {
            ...
            try
            {
                // Read the raw document from MongoDB — avoids object? deserialization issues
                var rawCollection = _db.Approvals.Database.GetCollection<MongoDB.Bson.BsonDocument>("approvals");
                var rawDoc = await rawCollection
                    .Find(MongoDB.Driver.Builders<MongoDB.Bson.BsonDocument>.Filter.Eq("_id", MongoDB.Bson.ObjectId.Parse(id)))
                    .FirstOrDefaultAsync();

                if (rawDoc != null && rawDoc.Contains("metadata"))
                {
                    var meta = rawDoc["metadata"].AsBsonDocument;
                    if (meta.Contains("productId"))
                        productId = meta["productId"].ToString();
                }
            }
        ...
        if (!string.IsNullOrWhiteSpace(approval.RequestedBy))
        {
            await _db.Notifications.InsertOneAsync(new Notification
            {
                UserId = approval.RequestedBy,
                Title = "Request " + (req.Status == "approved" ? "Approved" : "Rejected"),
                Message = $"Your request for '{approval.Action.Replace("_", " ")}' has been {req.Status}.",
